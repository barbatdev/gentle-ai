...
6. **No `Co-Authored-By` trailers** — never add AI attribution to commits; no force-push to protected main/master.
...
6. Open a PR referencing the issue
   → Declare exactly ONE type:* result in the PR body; use the canonical issue-creation workflow contract before any PR-label mutation
...
Closes #<N>
<!-- Or use `Refs #<N>` for a visible, non-closing link to an approved issue in the base repository. -->
