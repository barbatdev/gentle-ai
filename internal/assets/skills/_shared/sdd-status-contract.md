...
- When the session artifact store is `openspec` or `hybrid` and the `gentle-ai` binary is available, prefer `gentle-ai sdd-status [change] --cwd <repo> --json --instructions` for read-only status and `gentle-ai sdd-continue [change] --cwd <repo>` for dispatcher output. When the store is `engram`, do not invoke those OpenSpec dispatcher commands.
- The native dispatcher reads only OpenSpec file artifacts and emits `artifactStore: openspec`; it cannot observe Engram-backed changes. Treat dispatcher status as authoritative only when the selected artifact store is `openspec` or `hybrid`. When the selected store is `engram`, resolve artifact status from Engram (`mem_search` + `mem_get_observation` on the change topic keys) using the manual schema below.
...
- Verify routing parses only a strict leading `gentle-ai.verify-result/v1` or `gentle-ai.verify-result/v2` envelope, and compares measured requirement/scenario totals with actual specs. An executed report requires current test/build commands, zero exit codes, and output hashes. Human prose never controls readiness.
- A `v2` report may instead carry the `not_applicable` verdict, which records that the candidate owed no execution. That verdict FORBIDS every execution field rather than zeroing it, and requires a concrete `na_reason` plus `candidate_digest`, the identity of the classified content. Coverage MUST be reported as none assessed (`0/<total>`): a report stating that nothing was owed cannot also claim every requirement was covered. The totals still have to match the specs, and blockers and critical findings must still be zero.
- A `not_applicable` report attests only the content it names. `candidate_digest` covers the classified content with the report's own path excluded, because the report is stored inside the candidate it describes and a whole-tree identity would name a hash fixed point. Settlement recomputes that digest under the same exclusion, so a report emitted for one candidate cannot carry a later, drifted one.
- `v1` reports stay readable. Only the `v2` schema may express the `not_applicable` state, and a `v1` report that spells it, or carries its fields, is refused.
