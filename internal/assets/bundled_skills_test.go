...
func TestChainedPRSkillContract(t *testing.T) {
	...
	for _, required := range []string{
		...
		"`pr-body.md`", "do not modify the target repository's PR template",
	...
	portableEnd := strings.Index(portableDetails, "## GitHub Native Route")
	if portableEnd < 0 {
		t.Fatal("shipped chained-pr details missing GitHub native route")
	}
	portableDetails = portableDetails[:portableEnd]
